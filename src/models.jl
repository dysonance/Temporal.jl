...
depvar(mod::AR_Model)::Vector = mod.data.values[1+p:end,:]
indvar(mod::AR_Model)::Matrix = [ones(size(mod.data,1)-p) mod.data.values[1:end-p,:]]
...
type MA_Model <: TS_Model
end

type ARMA_Model <: TS_Model
end

type ARIMA_Model <: TS_Model
end

type VAR_Model <: TS_Model
end

type ARCH_Model <: TS_Model
end

type GARCH_Model <: TS_Model
end
